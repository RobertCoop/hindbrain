...
### Stop-lint: preferences enforced in pure prose

Hooks instrument the tool loop, not generation — a preference violated in plain response text (a banned opener in a drafted email, a phrasing the user corrected before) has no tool call to gate. Stop-lint closes the mechanically-checkable slice of that gap: at end of turn, the Stop hook scans `last_assistant_message` against active `preference` notes and, when a note **quotes** the phrase it bans (`avoid "moving forward"`, `never say "simply"`), a violation surfaces a declarative notice — the turn continues, so the agent can revise before the user ever sees the reply. Each note lints at most once per session (never a nag), at most 2 notices per turn, quarantined notes never lint, and lint surfacings feed activation like any other. `[lint] enabled = false` turns it off. Unquoted, judgment-call tone preferences remain out of scope by design — those belong in pinned context, not enforcement.
...
The bootstrap also seeds the association graph: the survey clusters candidates by evidence source, the judge proposes `related_to` links (dependency pairs at 0.6–0.8, cluster siblings at 0.3–0.45, and retrieval-backstop links for notes whose wording won't match the context where they matter), and the seeding agent creates them alongside the saves — so linked knowledge surfaces together from day one instead of waiting for the Hebbian consolidator to discover the structure.
